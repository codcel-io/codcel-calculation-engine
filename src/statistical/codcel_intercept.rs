...
pub fn codcel_intercept(
    ...
    let sum_x: f64 = known_xs.iter().compensated_sum();
    let sum_y: f64 = known_ys.iter().compensated_sum();
    let sum_xy: f64 = known_xs
        .iter()
        .zip(&known_ys)
        .map(|(&x, &y)| x * y)
        .compensated_sum();
    let sum_x2: f64 = known_xs.iter().map(|&x| x * x).compensated_sum();
