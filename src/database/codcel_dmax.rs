...
use crate::statistical::codcel_max::codcel_max;
use crate::value::Value;
use crate::value_format::ValueFormat;
use std::error::Error;

/// Excel-compatible `DMAX(database, field, criteria)`.
/// Maximum of `field` over matched rows. Empty match ⇒ 0.0 (matches Excel).
pub fn codcel_dmax(
    database: Value,
    field: Value,
    criteria: Value,
    ...
    Ok(Value::F64(codcel_max(values)?))
}
...
mod tests {
    ...
    fn dmax_basic() {
        ...
        let result = codcel_dmax(db(), Value::String("Salary".into()), crit, &vf()).unwrap();
        assert_eq!(result, Value::F64(100.0));
    }

    #[test]
    fn dmax_no_match_returns_zero() {
        let crit = Value::AreaValue(vec![
            vec![Value::String("Dept".into())],
