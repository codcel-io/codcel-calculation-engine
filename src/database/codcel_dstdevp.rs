...
/// Excel-compatible `DSTDEVP(database, field, criteria)`.
/// Population standard deviation of `field` over matched rows (n divisor).
/// Zero numeric values ⇒ `#NUM!`.
pub fn codcel_dstdevp(
    database: Value,
    field: Value,
    criteria: Value,
...
mod tests {
    ...
    fn dstdevp_basic() {
        ...
        let result = codcel_dstdevp(db(), Value::String("V".into()), crit, &vf()).unwrap();
        if let Value::F64(v) = result {
            // STDEV.P of {2,4,4,4,5,5,7,9} = 2.0
    ...
    #[test]
    fn dstdevp_no_match_returns_num_error() {
        let crit = Value::AreaValue(vec![
            vec![Value::String("G".into())],
