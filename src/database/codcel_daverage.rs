...
use crate::statistical::codcel_average::codcel_average;
use crate::value::Value;
use crate::value_format::ValueFormat;
use std::error::Error;

/// Excel-compatible `DAVERAGE(database, field, criteria)`.
/// Returns the arithmetic mean of `field` over matched rows. Empty match ⇒ 0.0.
pub fn codcel_daverage(
    database: Value,
    field: Value,
    criteria: Value,
    ...
    Ok(Value::F64(codcel_average(values)?))
}
...
mod tests {
    ...
    fn daverage_basic() {
        ...
        let result = codcel_daverage(db(), Value::String("Salary".into()), crit, &vf()).unwrap();
        assert_eq!(result, Value::F64(90.0));
    }

    #[test]
    fn daverage_no_match() {
        let crit = Value::AreaValue(vec![
            vec![Value::String("Dept".into())],
