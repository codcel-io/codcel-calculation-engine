...
/// Excel-compatible `DCOUNT(database, field, criteria)`.
/// Counts numeric cells in `field` across matched rows. If `field` is omitted
/// (Value::None or an empty string), counts matched rows numerically.
pub fn codcel_dcount(
    database: Value,
    field: Value,
    criteria: Value,
    ...
    let count = matched
        ...
        .filter(|cell| cell.f64(value_format).is_ok())
        .count() as i32;
...
mod tests {
    ...
    fn dcount_numeric_only() {
        ...
        let result = codcel_dcount(db(), Value::String("Salary".into()), crit, &vf()).unwrap();
    ...
    #[test]
    fn dcount_field_omitted_counts_rows() {
        let crit = Value::AreaValue(vec![
            vec![Value::String("Dept".into())],
            vec![Value::String("Eng".into())],
        ]);
        let result = codcel_dcount(db(), Value::None, crit, &vf()).unwrap();
